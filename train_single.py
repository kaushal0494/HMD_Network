...
def main(opt, device_id):
    ...
    # check decoder has same layers as question_init encoder for matching dim  
    assert opt.question_init_layers == opt.dec_layers 
    ...
    n_feats = first_dataset.n_feats
    
    # Load fields generated from preprocess phase.
    fields = _load_fields(first_dataset, data_type, opt, checkpoint, n_feats)
    
    # Report src/tgt features
    src_features, tgt_features, qu_features, ans_features = _collect_report_features(fields)
    ...
    for j, feat in enumerate(tgt_features):
        logger.info(' * tgt feature %d size = %d'
                    % (j, len(fields[feat].vocab)))
    for j, feat in enumerate(qu_features):
        logger.info(' * question feature %d size = %d'
                    % (j, len(fields[feat].vocab)))  
    for j, feat in enumerate(ans_features):
        logger.info(' * answer feature %d size = %d'
                    % (j, len(fields[feat].vocab)))    
    ...
    trainer = build_trainer(opt, device_id, model, fields,
                            optim, data_type, n_feats, model_saver=model_saver)
